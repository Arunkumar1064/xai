...
# ============= PAGE: COUNTERFACTUAL ANALYSIS =============
    ...
    with st.spinner("Generating counterfactuals..."):
        ...
        y_train_df = pd.DataFrame(y_train, columns=['Recommended_Crop'], index=X_train.index)
        dice_data = pd.concat([X_train, y_train_df], axis=1)
        
        # Now inverse transform labels (they are still valid)
        # dice_data['Recommended_Crop'] = le.inverse_transform(dice_data['Recommended_Crop'])
        # KEEP as integers to match model output!
        ...
        try:
            ...
            query_instance = X_test.iloc[sample_idx:sample_idx+1]
